...
import { cancelMembershipAction, changePlanAction } from "@/server/actions/billing";
          ...
          return (
            <li key={plan.tier} className={clsx("card flex flex-col p-6", current && "ring-2 ring-pine")}>
              ...
              {current ? (
                ...
                >
                  {plan.priceMonthly === 0 ? "Move to Free" : `Choose ${plan.name}`}
                </button>
              )}
            </li>
          );
        ...
        <div className="card flex flex-wrap items-center justify-between gap-4 p-5">
          ...
          {!cancelling && (
            <button
              ...
              onClick={() =>
                startTransition(async () => {
                  await cancelMembershipAction(true);
                  router.refresh();
                })
              }
            >
