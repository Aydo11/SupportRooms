...
export async function changePlanAction(tier: MembershipTier) {
  ...
  await notifyCompany(companyId, {
    ...
    body: `Your plan is now ${tier.toLowerCase()}.`,
...
/**
 * Sponsored placement packages. Prices are placeholders until a billing driver
 * is configured. The bid decides the order of the sponsored slots on page one;
...
export async function featureListingAction(listingId: string, pkg: SponsorPackage = "MONTH") {
  ...
  if (!usingCredit) {
    await db.payment.create({
      data: {
        ...
        amount: plan.amount,
        status: "PENDING",
        description: `Sponsored placement, ${plan.label}`,
      },
    });
    // With a live billing driver this would return a checkout URL instead of
    // activating straight away.
  }
  ...
  return {
    ok: true,
    message: usingCredit
      ? `Sponsored for ${plan.label}, using one of your included slots.`
      : `Sponsored for ${plan.label}. The payment is recorded as pending until billing is live.`,
  };
}
